...
module PetitCheerTypeConverter =
    ...
    let toString (petitCheerEffect) =
        ...
        | AttributeCheerType (attr, mode) ->
            SkillAttributeTypeConverter.toString (attr)
            + "の"
            + ModeConverter.toString (mode)
            + "UP"
        | GradeCheerType (grade, mode) ->
            GradeConverter.toString (grade)
            + "の"
            + ModeConverter.toString (mode)
            + "UP"
        ...
        | FacilityBuildUpType (targetFacility) ->
            FacilityConverter.toString (targetFacility)
            + "の効果"
            + "UP"
    ...
    let fromString (petitCheerEffectStr: string) =
        ...
        let attributeCheerPattern =
            "((?:COOL|POP|SWEET|全)ﾀｲﾌﾟ)の(攻援|守援|攻守)(?:大|小)?UP"
        ...
        | ParseRegex attributeCheerPattern [ skillAttr; mode ] ->
            AttributeCheerType(SkillAttributeTypeConverter.fromString (skillAttr), ModeConverter.fromString (mode))
        | ParseRegex gradeCheerPattern [ grade; mode ] ->
            GradeCheerType(GradeConverter.fromString (grade), ModeConverter.fromString (mode))
...
module PetitSkillTypeConverter =
    let toString (petitSkill) =
        ...
        | AttributeSkillType (skillAttr, mode) ->
            SkillAttributeTypeConverter.toString (skillAttr)
            + "のぷちガール"
            + ModeConverter.toString (mode)
            + "UP"
    ...
    let fromString (petitSkillStr) =
        ...
        let attributeSkillPattern =
            "((?:COOL|POP|SWEET|全)ﾀｲﾌﾟ)のぷちｶﾞｰﾙ(攻援|守援|攻守)UP"

        match petitSkillStr with
        | ParseRegex attributeSkillPattern [ skillAttr; mode ] ->
            AttributeSkillType(SkillAttributeTypeConverter.fromString (skillAttr), ModeConverter.fromString (mode))
