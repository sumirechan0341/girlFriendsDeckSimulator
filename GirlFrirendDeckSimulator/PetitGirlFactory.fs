...
module PetitGirlFactory =
    type PetitGirlElements = JsonProvider<""" {"girlName":"上条るい","eventName":"猫コス18","rarity":"SR","attribute":"COOL","attack":13674,"defence":11603,"petitCheerEffects":[{"CheerType":"COOLﾀｲﾌﾟの攻援UP","CheerEffectNum":9.5,"CheerEffectMode":"UP"},{"CheerType":"本命ｶﾞｰﾙの攻守UP","CheerEffectNum":11,"CheerEffectMode":"UP"},{"CheerType":"誕生日のｶﾞｰﾙの攻守UP","CheerEffectNum":19,"CheerEffectMode":"UP"},{"CheerType":"ﾎﾜｲﾄﾎﾞｰﾄﾞの効果UP","CheerEffectNum":120,"CheerEffectMode":"UP"}],"petitSkillEffect":{"petitSkillEffectType":"COOLﾀｲﾌﾟのぷちｶﾞｰﾙ攻守UP","petitSkillEffectNum":17,"petitSkillEffectMode":"UP"},"selectionBonus":"↑スレンダーズボルケーノ↑"} """>
    let petitGirlFactoryFromJson(petitGirlJson) = 
        let petitGirlElements = PetitGirlElements.Parse(petitGirlJson)
        {
            ...
            selectionBonus = 
                match petitGirlElements.SelectionBonus with
                | "" -> None
                | _ -> Some(SelectionBonus(petitGirlElements.SelectionBonus));
        }
        
    //jsonの末尾に改行とかあると失敗する
    // なんとかするかも
    let petitGirlList = Seq.map(petitGirlFactoryFromJson) (File.ReadLines("..\..\PetitGirl.json"))